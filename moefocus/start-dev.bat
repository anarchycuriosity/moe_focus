...
if not exist "node_modules\electron\dist\electron.exe" (
    ...
    echo.
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$pkg = Get-Content 'node_modules\electron\package.json' -Raw ^| ConvertFrom-Json; $ver = $pkg.version; Write-Host ('[*] Target: Electron v' + $ver) -ForegroundColor Cyan; $url1 = 'https://npmmirror.com/mirrors/electron/v' + $ver + '/electron-v' + $ver + '-win32-x64.zip'; $url2 = 'https://github.com/electron/electron/releases/download/v' + $ver + '/electron-v' + $ver + '-win32-x64.zip'; $zip = $env:TEMP + '\electron-v' + $ver + '.zip'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $ok = $false; foreach ($u in @($url1,$url2)) { try { Write-Host ('[*] Trying: ' + $u) -ForegroundColor Gray; Invoke-WebRequest -Uri $u -OutFile $zip -ErrorAction Stop; $ok = $true; break } catch { Write-Host '    Failed, trying fallback...' -ForegroundColor DarkYellow } }; if (-not $ok) { Write-Host '[X] All download URLs failed' -ForegroundColor Red; exit 1 }; Write-Host '[*] Extracting...' -ForegroundColor Cyan; Expand-Archive -Path $zip -DestinationPath 'node_modules\electron\dist' -Force; Remove-Item $zip; 'electron.exe' ^| Out-File -FilePath 'node_modules\electron\path.txt' -Encoding ascii -NoNewline; if (-not (Test-Path 'node_modules\electron\dist\electron.exe')) { Write-Host '[X] Extraction failed' -ForegroundColor Red; exit 1 }; Write-Host '[OK] Electron installed' -ForegroundColor Green"
    if %ERRORLEVEL% NEQ 0 (
        echo.
