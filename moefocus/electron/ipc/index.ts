...
function registerSettingsHandlers(): void
{
  ...
  ipcMain.handle('settings:set', (_event, key, value) =>
  {
    ...
    // 设置变更后重调度定时任务（使新时间立即生效，无需重启应用）
    const scheduler_keys = [
      'diary.autoGenerateTime',
      'email.reminderTime',
      'email.blogReminderTime',
      'email.blogReminderDay',
      'email.blogReminderEnabled'
    ]
    if (scheduler_keys.includes(key))
    {
      scheduler_service.restart()
      console.log('[settings] scheduler restarted due to', key, 'change')
    }
...
function registerEmailHandlers(): void
{
  ...
  {
    return scheduler_service.trigger_email_now()
  })

  // 手动测试：发送每周博客提醒（委托给 scheduler_service）
  ipcMain.handle('email:sendTestBlogReminder', async () =>
  {
    return scheduler_service.trigger_blog_now()
  })

  // 手动触发：立即生成今日日记并用 Typora 打开
  ipcMain.handle('scheduler:triggerDiary', async () =>
  {
    return scheduler_service.trigger_diary_now()
  })
