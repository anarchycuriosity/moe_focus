...
本仓库是 MoeFocus 的**源代码仓库**。如果你只想使用应用（不参与开发），按以下步骤操作：

### 方式一：双击启动（推荐）

直接双击 `moefocus/start-dev.bat`，脚本会自动安装依赖并启动开发服务器。
...
> **注意**：项目在 Windows PowerShell 环境下开发。WSL 中运行可能出现路径问题。

### 我拿到的就是这个仓库，然后呢？

1. `git clone` 本仓库到本地
2. 进入 `moefocus/` 目录，运行 `npm install`
3. 运行 `npm run dev` 启动应用（或直接双击 `start-dev.bat`）
4. 应用启动后：
...
就这么简单。不需要额外安装数据库或配置环境 —— SQLite 内嵌在应用中，所有数据自动管理。
...
1. 应用在 `%APPDATA%/moefocus/` 下自动 `git init`，用你配置的远程仓库作为 origin
2. 手动同步：设置 → GitHub → 点击 Commit → Push 上传 / Pull 下载
3. `.gitignore` 自动排除 `*.db`（SQLite 原始数据库不同步，而是通过 JSON 导入导出）
4. 实际同步的文件：
   - `sums/`：日记 Markdown 文件（`YYYY-MM-DD.md`）
   - `data/`：JSON 格式的应用数据导出（任务库、专注记录、设置等）

### 配置步骤

1. 在 GitHub 创建一个**空的私有仓库**（例如 `moefocus-data`），**不要**勾选 "Add a README"
2. 打开 MoeFocus → 设置 → GitHub 标签页
3. 输入仓库地址（如 `https://github.com/你的用户名/moefocus-data.git`）→ 点击「应用远程地址」
4. 点击「检查同步状态」确认连接正常
5. 使用 Commit / Push / Pull 按钮手动同步
6. 确保系统已配置 Git 凭证管理器：
   ```
...
写下日记 + 专注记录                                          写下日记 + 专注记录
    │                             │                             │
    └─ Commit + Push ───→  sums/data  ───→ Pull ─── 自动导入到本地
                          合并双方数据
    ←── Pull ─── sums/data ←─── Commit + Push ───────────┘
```
...
- 自定义壁纸文件（每台电脑独立管理——文件体积大，且路径因电脑而异）
